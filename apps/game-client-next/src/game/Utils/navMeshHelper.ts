...
function createConvexRegionHelper(navMesh: NavMesh, scene: Scene): Mesh {
    ...
    for (let region of regions) {
        ...
        if (!edge) continue; // Skip if edge is null
        ...
        while (currentEdge && currentEdge !== region.edge) {
...
function createGraphHelper(scene: Scene, graph: any, nodeSize = 1, nodeColor = "#4e84c4", edgeColor = "#ffffff") {
    const nodes: any[] = [];
...
export async function loadNavMeshFromString(fileNameNavMesh: string): Promise<NavMesh> {
...
export { NavMesh, createConvexRegionHelper, createGraphHelper };
